...
*NB: if you have multiple animal movement projects, these should have completely separate directories*
...
Throughout the script, we'll be saving files using a species code as a file/folder identifier. Our code is taken from the first letters of the species name (**R**ed **F**ooted **B**ooby) but can be anything you choose (shorter the better as long as it's unique per species). Let's define this object first for consistency:
...
The default CRS for this workflow is the [Spherical Mercator projection](https://epsg.io/3857) — (`crs = 3857`), which is used by Google Maps/OpenStreetMap and works worldwide. However, this CRS can over-estimate distance calculations in some cases, so it's important to consider the location and scale of your data (e.g., equatorial/polar/local scale/global scale) and choose a projection system to match. Other options include (but are not limited to) UTM, and Lambert Azimuthal Equal-Area (LAEA) projections.
