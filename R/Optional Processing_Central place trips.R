...
#'  - Option C = define trips by latitude & longitude
...
## This script runs using df_diagnostic as an example
## If you have run other optional processes (e.g., df_resampled) then use your most recent data here

filepath <- here("TestDataOutputs","WorkingDataFrames","RFB_diagnostic.csv")
...
shapefilepath <- here("TestData","RFBTest_CPshape","RFBTest_CPshape.shp")
...
#--------------------------------------------------------#
## 3. Option C = define trips by latitude & longitude ####
#--------------------------------------------------------#

## Here, we define trips using latitude and longitude around the CP
## This is not best practice if more precise CP locations are known
## skip this step if using Option A or B
...
# fill with a new entry for each population
# "population" = bounding box limits
# it is important to use the same population names as in the tracking data for subsequent conditional formatting
ls_CP_boxes <- list(
  "DG" = c(xmin = 72.429, xmax = 72.458, ymin = -7.258, ymax = -7.229),
  "NI" = c(xmin = 72.304, xmax = 72.328, ymin = -5.688, ymax = -5.676)
)
...
# convert list of extents to polygons
CP_boxes <- ls_CP_boxes %>%
  lapply(., function(x) st_sf(CP = "CP", geometry = st_as_sfc(st_bbox(x, crs = 4326)))) %>%
  rbindlist(., idcol = "Population") %>%
  st_as_sf() %>%
  st_transform(crs = 3857)

# quick plot of the colony polygons
ggplot(CP_boxes)+
  geom_sf()+
  theme_light()
...
df_CP <- df_diagnostic %>% 
  st_as_sf(., coords=c("Lon","Lat"), crs = 4326, remove=FALSE) %>% 
  st_transform(crs = 3857) %>%
  st_join(., CP_boxes, join = st_intersects) %>%
  mutate(atCP = ifelse(CP %in% ("CP") & .$Population.x == .$Population.y, "Yes", "No")) %>% # atCP = Yes when points intersect with the population-specific polygon
  select(-CP, -Population.y) %>% # remove common column
  rename(Population = Population.x)
...
grouping_factors_indlevel <- c("ID", "TripID")
...
df_tripmetrics_summary_pop <- df_tripmetrics %>%
  group_by(across(grouping_factors_poplevel)) %>%
  summarise(Trip_duration_mean = mean(as.numeric(Trip_duration, units = sampleRateUnits), na.rm = T),
            Trip_duration_se = se(as.numeric(Trip_duration, units = sampleRateUnits)),
            Total_distance_mean = mean(Total_distance),
            Total_distance_se = se(Total_distance),
            Max_distance_mean = mean(Max_distance),
            Max_distance_se = se(Max_distance))
...
map_trips <- map_base + 
  ...
  facet_wrap(~ ID, ncol = n_distinct(df_diagnostic$ID))
map_trips
...
filepath_dftrips_out <- here("TestDataOutputs","WorkingDataFrames") 
